...
Modelo ${prefix}verify untuk para começar a usar bot.`
  ...
  only: {
    ...
    ownerB: '*☒* Este recurso é para *PAULO*',
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('PAULO', 'CURA LÉSBICA KKKK')} ${ran} -o ${ran}`, async (error) => {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('PAULO', 'CURA LÉSBICA KKK')} ${ran} -o ${ran}`, async (error) => {
